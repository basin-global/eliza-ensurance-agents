...
export const getBalanceAction: TokenboundAction = {
  name: 'TBA_GET_BALANCE',
  description: 'Get all token balances for the Tokenbound Account',
  ...
  async handler(runtime, message, state, options, callback) {
    try {
      ...
      if (callback) {
        callback({
          text: `Error getting balances: ${error.message}`,
  ...
  similes: ['GET_BALANCE', 'CHECK_BALANCE', 'SHOW_TOKENS', 'VIEW_WALLET'],
...
function formatBalances(data: SimpleHashBalance): string[] {
  ...
  return lines.length ? lines : ['No balances found'];
}